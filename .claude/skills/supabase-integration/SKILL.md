...
### ⚠️ 주의할 점
- 본문에 SQL DDL / 제약 / 인덱스 / 정책 / 트리거 / 훅 코드 등장 금지. 그건 부록 영역.
- § 1.5 의 "DB 스펙 미리보기" 만 예외. 컬럼명·PG 타입·null 만 노출 (4컬럼). 그 이상 디테일은 `appendix-db-schema.md`.
...
5. `docs/{project}/04-data-bridge.md` 작성. **3 가지 질문에 답하는 구조** 강제. 디자이너가 가장 쉽게 이해하도록:
   - **§ 1. 데이터 모델은 어떤 DB 테이블이 되나?** (한 줄 매핑)
     ...
     - 디자이너가 펼쳤을 때 압박 0.
   - **§ 1.5. DB 스펙 미리보기 (간단)** (테이블별 컬럼 표)
     - ux-flow 데이터 모델 카드의 "필드 표" 를 PG 컬럼명·타입으로 변환만. 테이블당 표 1개.
     - **4컬럼 강제**: 컬럼 / 타입 / null / 설명. 제약·인덱스·기본값·정책·트리거 금지 (부록 영역).
     - 타입은 PG 기본 표기 (`uuid`, `text`, `int`, `boolean`, `timestamptz`, `jsonb`, `text[]`). 도메인 타입·체크 제약 금지.
     - FK 는 타입 칸에 `uuid (→ {table}.id)` 한 줄로. ON DELETE / ON UPDATE 는 부록.
     - 자동 컬럼 (id / created_at / updated_at) 은 표 하단 1줄로 묶음 ("자동: id (uuid PK), created_at, updated_at").
     - 카드 필드 ↔ DB 컬럼 명명 변환 규칙: camelCase → snake_case. 변환 충돌 시 사용자에게 묻기.
     - 목적: 디자이너+개발자가 한 화면에서 "어떤 컬럼이 생기는가" 합의. DDL 부록을 펼치지 않아도 됨.
...
6. **3 단답 승인 게이트**:
   ...
   - "§ 1.5 DB 스펙 미리보기의 컬럼 / 타입 OK? (제약·인덱스는 부록)"
